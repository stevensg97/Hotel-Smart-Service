...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Group.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Group.query(buildQuery({ model: Group, filters }))
  ...
  count: (params) => {
    ...
    return Group.query(buildQuery({ model: Group, filters: _.pick(filters, 'where') })).count();
  },
